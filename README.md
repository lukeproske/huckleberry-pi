...
- The screen works for the booting process but goes black at the login screen. After blindly typing the password, the screen starts working again. This is most likely a software issue caused by Kali Linux, KDE Plasma, or the display manager. I am attempting to discover a solution, but the Cyberdeck is still perfectly usable. For now, lets just call it a "security feature." UPDATE: I finally managed to fix this issue. My solution is described in the "dsi-sddm-fix.md" file.
